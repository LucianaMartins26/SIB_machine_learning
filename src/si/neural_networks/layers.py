...
from SIB_machine_learning.src.si.neural_networks.optimizers import Optimizer
...
if __name__ == '__main__':
    ...
    # Cria uma instância de DenseLayer
    layer = DenseLayer(n_units=10, input_shape=(5,))

    # Inicializa a camada com um otimizador SGD
    layer.initialize(optimizer=SGD(learning_rate=0.01))

    # Imprime o número de parâmetros da camada
    print("Número de Parâmetros da Camada Dense:", layer.parameters())

    # Gera dados de entrada aleatórios
    input_data = np.random.rand(1, 5)

    # Realiza a propagação direta (forward propagation) na camada
    output_data = layer.forward_propagation(input_data, training=True)

    # Imprime a saída da camada
    print("Saída da Camada Dense:", output_data)

    # Gera um erro de saída aleatório (apenas para fins de demonstração)
    output_error = np.random.rand(1, 10)

    # Realiza a propagação inversa (backward propagation) na camada
    input_error = layer.backward_propagation(output_error)

    # Imprime o erro de entrada
    print("Erro de Entrada da Camada Dense:", input_error)
